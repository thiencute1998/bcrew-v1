                    ...
                    <ul class="nav nav-tabs tab_show_title">
                        @foreach($products as $product)
                            <li @if($product->slug == $slug) class="active" @endif>
                                <a href="{{route('virtual_staging_slug', ['slug'=> $product->slug])}}">{{$product->name}}</a>
                            </li>
                        @endforeach
                    </ul>
                                    ...
                                    @foreach($productDetail->productImages as $detail)
                                        <div class="item">
                                            <div class="entry-image-gallery">
                                                <img src="{{asset('upload/admin/services/virtual_staging/' . $detail->file_name_1)}}">
                                                <img src="{{asset('upload/admin/services/virtual_staging/' . $detail->file_name_2)}}">
                                            </div>
                                        </div>
                                    @endforeach
